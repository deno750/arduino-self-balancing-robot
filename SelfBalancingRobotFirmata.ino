...
void setup()
{
    ...
    // initialize device
    Firmata.sendString(F("Initializing I2C devices..."));
    mpu.initialize();

    // verify connection
    Firmata.sendString(F("Testing device connections..."));
    Firmata.sendString(mpu.testConnection() ? F("MPU6050 connection successful") : F("MPU6050 connection failed"));

    // load and configure the DMP
    Firmata.sendString(F("Initializing DMP..."));
    ...
    if (devStatus == 0)
    {
        ...
        Firmata.sendString(F("Enabling DMP..."));
        mpu.setDMPEnabled(true);

        // enable Arduino interrupt detection
        Firmata.sendString(F("Enabling interrupt detection (Arduino external interrupt 0)..."));
        attachInterrupt(0, dmpDataReady, RISING);
        mpuIntStatus = mpu.getIntStatus();

        // set our DMP Ready flag so the main loop() function knows it's okay to use it
        Firmata.sendString(F("DMP ready! Waiting for first interrupt..."));
        dmpReady = true;
    ...
    else
    {
        ...
        Firmata.sendString(F("DMP Initialization failed (code " + devStatus + ")"));
    }
}
...
void loop() {
  ...
  // if programming failed, don't try to do anything
    ...
    if ((mpuIntStatus & 0x10) || fifoCount == 1024)
    {
        ...
        mpu.resetFIFO();
        Firmata.sendString(F("FIFO overflow!"));
